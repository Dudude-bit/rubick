/**
 * Is the connected Prometheus the right one, and does it have what the app
 * asks it for.
 *
 * A probe proves the address speaks PromQL; it cannot tell you the
 * Prometheus you reached is watching this cluster at all. The
 * organisation's central Prometheus, scraping four clusters and none of
 * them this one, connects, reads as healthy, and draws charts about
 * somebody else's pods. So this tab is one comparison and one inventory:
 * the nodes it knows against the nodes this cluster has, and the metric
 * families the app's queries are built on.
 */
...
export default function Connection() {
  ...
  // No address is no comparison to draw: the page's only content would be
  // the coverage query's failure to reach anything.
  if (saved.data === null) {
    return (
      <Section className="max-w-[64ch] py-8">
        <p className="text-xs text-fg-mut">
          {t("monitors", "noAddress")} {edit}
        </p>
      </Section>
    );
  }
      ...
      {saved.data && (
        <p className="text-xs text-fg-mut">
          <span className="font-mono text-fg">
            {saved.data.url.replace(/^https?:\/\//, "").replace(/\/+$/, "")}
          </span>
