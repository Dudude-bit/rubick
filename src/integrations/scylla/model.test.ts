...
describe("readScyllaCluster", () => {
  ...
  it("reads an upgrade in progress from status.upgrade and the rack it is on", () => {
    ...
    expect(upgrading?.detail).toContain("2025.1.3 → 2025.2.1");
    expect(upgrading?.detail).toContain("rack eu-1b");
  });
  ...
  /** No status is not a healthy cluster: it is an operator that has not spoken. */
  it("says the operator has written nothing rather than drawing zeros", () => {
    ...
    expect(read.racks[0].ready).toBe(0);
  });
